...
void font_init() {
    // Nothing needed -- font is compiled in
}

int font_char_width(int size) {
    switch (size) {
        case FONT_SMALL:  return 8;
        case FONT_MEDIUM: return 9;   // 8px chars + 1px spacing
        case FONT_LARGE:  return 12;  // scaled 1.5x
        default: return 8;
    }
}
...
int font_char_height(int size) {
    switch (size) {
        case FONT_SMALL:  return 8;
        case FONT_MEDIUM: return 12;  // 8px + 4px extra line spacing
        case FONT_LARGE:  return 16;  // scaled 2x
        default: return 8;
    }
}

// Draw a character from the 8x8 base font, scaled
void font_draw_char(int x, int y, char c, uint32_t fg, uint32_t bg, int size) {
    if (c < 32 || c > 126) c = '?';
    const uint8_t* glyph = font_8x8[c - 32];

    int scale = 1;
    if (size == FONT_LARGE) scale = 2;
        ...
        for (int col = 0; col < 8; col++) {
            ...
            for (int sy = 0; sy < scale; sy++)
                for (int sx = 0; sx < scale; sx++)
                    fb_putpixel(x + col * scale + sx, y + row * scale + sy, color);
        }
    }
    // For MEDIUM, add extra spacing below (filled with bg)
    if (size == FONT_MEDIUM) {
    ...
    for (int row = 0; row < 8; row++) {
        ...
        for (int col = 0; col < 8; col++) {
            if (bits & (0x80 >> col)) {
                for (int sy = 0; sy < scale; sy++)
                    for (int sx = 0; sx < scale; sx++)
                        fb_putpixel(x + col * scale + sx, y + row * scale + sy, fg);
            }
        }
    }
}
...
void font_draw_string(int x, int y, const char* str, uint32_t fg, uint32_t bg, int size) {
    ...
    while (*str) {
        font_draw_char(x, y, *str, fg, bg, size);
        x += cw;
        str++;
    }
}
...
void font_draw_string_nobg(int x, int y, const char* str, uint32_t fg, int size) {
    int cw = font_char_width(size);
    while (*str) {
        font_draw_char_nobg(x, y, *str, fg, size);
        x += cw;
        str++;
    }
}
